		...
		if event is InputEventMouseMotion:
			...
			if event.pressed:
				...
				if have_affected_lattice:
					...
					undo_redo.add_do_method(edit_node, "add_lattice", diff, 1.0)
					undo_redo.add_undo_method(edit_node, "add_lattice", diff, -1.0)
...
func update_raycast():
	...
	var space := edit_node.get_world_3d().direct_space_state
